...
class Pintura{
    constructor({ id_pintura,id_usuario,titulo,descripcion, imagen,fecha_pintura,likes,created_by, created_at ,updated_by,updated_at,deleted_by,deleted_at,deleted }) {
      ...
      static async getById(id) {
        const connection = await db.createConnection();
        const [rows] = await connection.query(
          "SELECT id_pintura,id_usuario,titulo,descripcion, imagen,fecha_pintura,likes,created_by, created_at ,updated_by,updated_at,deleted_by,deleted_at,deleted FROM pinturas WHERE id_pintura=? AND deleted=0 ;",
          [id]
        );
        connection.end();
        if (rows.length > 0) {
